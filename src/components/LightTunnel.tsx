...
export default function LightTunnel({
  ...
  useEffect(() => {
    ...
    // Detect low-end devices (e.g. mobile or low CPU cores) for optimized DPR capping
    const isLowEnd =
    ...
    const maxDpr = isLowEnd ? 1.0 : Math.min(window.devicePixelRatio || 1, 1.5);
    ...
    const effectiveCables = isLowEnd ? Math.min(cableCount, 16) : cableCount;
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uCableCount: { value: effectiveCables },
        ...
        uCableColor: { value: new Float32Array(cable) },
        uPulseColor: { value: new Float32Array(pulse) },
        uTunnelColor: { value: new Float32Array(tunnel) },
        uTunnelOpacity: { value: tunnelOpacity },
        uGrain: { value: isLowEnd ? 0.0 : grain ? 1.0 : 0.0 },
        uGrainIntensity: { value: isLowEnd ? 0.0 : grainIntensity },
      },
    });
